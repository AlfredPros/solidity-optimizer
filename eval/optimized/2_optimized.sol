...
library EVMScriptCreator {
    ...
    function createEVMScript(
        ...
        for (uint256 i; i < _evmScriptCallData.length; ++i) {
    ...
    function createEVMScript(
        ...
        for (uint256 i; i < _methodIds.length; ++i) {
            _evmScript = bytes.concat(
                _evmScript,
                abi.encodePacked(
                    _to,
...
contract LimitsChecker is AccessControl {
    ...
    constructor(
        ...
        for (uint256 i; i < _setParametersRoleHolders.length; ++i) {
            _setupRole(SET_PARAMETERS_ROLE, _setParametersRoleHolders[i]);
        }
        for (uint256 i; i < _updateSpentAmountRoleHolders.length; ++i) {
...
contract AllowedRecipientsRegistry is LimitsChecker {
        ...
        )
    {
        ...
        for (uint256 i; i < _addRecipientToAllowedListRoleHolders.length; ++i) {
            _setupRole(
                ADD_RECIPIENT_TO_ALLOWED_LIST_ROLE,
                _addRecipientToAllowedListRoleHolders[i]
            );
        }
        for (uint256 i; i < _removeRecipientFromAllowedListRoleHolders.length; ++i) {
            _setupRole(
